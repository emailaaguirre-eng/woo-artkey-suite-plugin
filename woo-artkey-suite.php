...
class Woo_ArtKey_Suite {
    ...
    $watch_video_url = esc_url($watch_video['url'] ?? '');
        ...
        <script>
        (function(){
            const root = document.currentScript ? document.currentScript.previousElementSibling : document;
            const page = root.closest('.artkey-wrap') || document;
            ...
            function openGallery(){ if (galModal) galModal.style.display = 'flex'; }
            function closeGallery(){ if (galModal) galModal.style.display = 'none'; }
            if (galModal) {
            ...
            function openVideos(){ if (vidModal) vidModal.style.display = 'flex'; }
            function closeVideos(){ if (vidModal) vidModal.style.display = 'none'; }
            if (vidModal) {
            ...
            function openWatchVideo(){
                if (watchModal) watchModal.style.display = 'flex';
                // Autoplay attempt: browsers often only permit autoplay when muted.
                if (watchPlayer && watchPlayer.play) {
                    try {
                        ...
                        if (p && typeof p.then === 'function') {
                            p.then(function(){
                                // Leave muted if that's what the browser requires; otherwise restore.
                                if (!wasMuted) {
                                    // Small delay helps some browsers stabilize playback state.
            ...
            document.addEventListener('keydown', function(e){
                if (e.key === 'Escape' || e.key === 'Esc') {
                    ...
                    closeWatchVideo();
                    closeLightbox();
                    if (msg1Modal) msg1Modal.style.display = 'none';
                    if (msg2Modal) msg2Modal.style.display = 'none';
                }
            });

            function openMsg1(){ if (msg1Modal) msg1Modal.style.display = 'flex'; }
            function closeMsg1(){ if (msg1Modal) msg1Modal.style.display = 'none'; }
            if (msg1Modal) msg1Modal.addEventListener('click', (e)=>{ if (e.target === msg1Modal) closeMsg1(); });
            if (msg1Close) msg1Close.addEventListener('click', closeMsg1);

            function openMsg2(){ if (msg2Modal) msg2Modal.style.display = 'flex'; }
            function closeMsg2(){ if (msg2Modal) msg2Modal.style.display = 'none'; }
            if (msg2Modal) msg2Modal.addEventListener('click', (e)=>{ if (e.target === msg2Modal) closeMsg2(); });
            if (msg2Close) msg2Close.addEventListener('click', closeMsg2);

            function showSign(){ if (signPanel) { signPanel.style.display = 'block'; signPanel.scrollIntoView({behavior:'smooth', block:'center'}); } }
            ...
            document.addEventListener('click', function(e){
                ...
                if (cta === 'gb_view') openModal();
                else if (cta === 'gb_sign') showSign();
                else if (cta === 'gallery') openGallery();
                else if (cta === 'video') openVideos();
                else if (cta === 'watch_video') openWatchVideo();
                else if (cta === 'messages_1') openMsg1();
                else if (cta === 'messages_2') openMsg2();
                else if (cta === 'imgup') toggleUpload('images');
                else if (cta === 'vidup') toggleUpload('videos');
            });
